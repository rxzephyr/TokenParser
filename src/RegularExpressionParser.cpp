...
Sequence<char> &RegularExpressionParser::getPostfixQueue()
{
	return postfixQueue;
}
...
void RegularExpressionParser::transferToPostfix(Sequence<char> inputQueue)
{
	Sequence<char> reverseStack;
...
//			reverseStack.AddFinal(element);
		...
		else if(element==CLUSTER)
		{
			for(;(!reverseStack.isEmpty())&&
				 (reverseStack[reverseStack.getUsage()-1]==CLOSURE||
				  reverseStack[reverseStack.getUsage()-1]==CLUSTER);)
			{
				char tmp=0;
		...
		else if(element==PARALLEL)
		{
			for(;(!reverseStack.isEmpty())&&
			   (reverseStack[reverseStack.getUsage()-1]==CLOSURE||
				reverseStack[reverseStack.getUsage()-1]==CLUSTER||
				reverseStack[reverseStack.getUsage()-1]==PARALLEL);)
			{
				char tmp=0;
				reverseStack.ExtractFinal(tmp);
				postfixQueue.AddFinal(tmp);
		...
		{
			char tmp=0;
			for(;!reverseStack.isEmpty()&&reverseStack[reverseStack.getUsage()-1]!=LEFT_BRACE;)
			{
				reverseStack.ExtractFinal(tmp);
				postfixQueue.AddFinal(tmp);
			}
...
{
	Sequence<char> postfixList(postfixQueue);
	Sequence<REBinaryClient<char> *> parseTreeList;
	char outChar=0;
...
void RegularExpressionParser::MakeFinalDFA()
{
	...
	{
		if(isFinalState[i])
			finalDFA.getSpecialVertexListR().AddFinal(i);//终止状态
